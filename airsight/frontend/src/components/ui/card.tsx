 ...
 *
 * AirSight uses SHARP corners (rounded-none) for chrome to honour the MarcVista
 * signature (the source MarcVista card used rounded-lg; we override to 0px here).
 */
import * as React from 'react';
import { cva, type VariantProps } from 'class-variance-authority';
import { cn } from '../../lib/cn';

const card = cva('block transition-all duration-md ease-out rounded-none', {
  variants: {
    variant: {
      default: 'bg-surface border border-line-soft hover:shadow-xs',
      elevated: 'bg-surface border border-line-soft shadow-xs hover:shadow-sm',
